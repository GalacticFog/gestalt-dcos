...
class LauncherSpecs extends PlaySpecification with Mockito with MockWSHelpers {
  ...
  case class TestModule(sseFactory: MarathonSSEClient.Factory, ws: WSClient) extends AbstractModule with ScalaModule with AkkaGuiceSupport {
...
//      bindActorFactory[MarathonSSEClient, MarathonSSEClient.Factory]
  ...
  abstract class WithConfig(config: (String,Any)*)
    ...
    val mockSSEClientFactory = mock[MarathonSSEClient.Factory]
    val mockSSEClient = TestProbe()
//    mockSSEClientFactory.apply() returns mockSSEClient
    val mockWSClient = mock[WSClient]
    val injector =
      new GuiceApplicationBuilder()
        .disable[modules.Module]
        .disable[play.api.libs.ws.ahc.AhcWSModule]
        .bindings(TestModule(mockSSEClientFactory, mockWSClient))
        .configure(config:_*)
        .injector
  }
  ...
  abstract class WithRoutesAndConfig(routes: MockWS.Routes, config: (String,Any)*)
    ...
    val mockSSEClientFactory = mock[MarathonSSEClient.Factory]
    val mockSSEClient = TestProbe()
//    mockSSEClientFactory.apply() returns mockSSEClient.ref
      ...
      mockSSEClient.setAutoPilot(new TestActor.AutoPilot {
        def run(sender: ActorRef, msg: Any): TestActor.AutoPilot = msg match {
          case MarathonSSEClient.LaunchAppRequest(app) if app.id.get.endsWith("/elasticsearch") =>
            testActor.tell(Json.obj(), sender)
            TestActor.KeepRunning
          case MarathonSSEClient.GetServiceStatus(ELASTIC) =>
            testActor.tell( ServiceInfo(
              service = ELASTIC,
              vhosts = Seq.empty,
