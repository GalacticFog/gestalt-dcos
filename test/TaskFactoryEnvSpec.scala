...
class TaskFactoryEnvSpec extends Specification {
  ...
  "GestaltTaskFactory" should {
    ...
    "support environment variables for ensemble versioning or override" in {
      ...
      gtf.getAppSpec(DATA(0), globals)     must haveImage(env("GESTALT_DATA_IMG").getOrElse(s"galacticfog/gestalt-data:dcos-${ensver}"))
      gtf.getAppSpec(DATA(1), globals)     must haveImage(env("GESTALT_DATA_IMG").getOrElse(s"galacticfog/gestalt-data:dcos-${ensver}"))
